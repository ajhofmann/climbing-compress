...
- Filename comparators are supported as exact-match checks too (for example `#name=clip.mp4`, `#name!=clip.mp4`), including aliases `#file...` and `#filename...`.
...
- Malformed `#name...` / `#file...` / `#filename...` comparator tags now surface clickable filename examples (`#name=clip.mp4`, `#name!=clip.mp4`) with include/exclude prefix + alias preservation.
- Malformed `#fps...` / `#w...` / `#h...` / `#ar...` / `#fc...` / `#res...` comparator tags now surface clickable video-meta examples (`#fps>=24`, `#fps=24..60`, `#w>=1080`, `#h>=1080`, `#ar>=1.3`, `#fc>=25`, `#res=1920x1080`), preserving long-form aliases when typed (`#framerate...`, `#width...`, `#height...`, `#frames...`, `#resolution...`).
...
- Comparator fragments like `#src=` / `#out=` / `#dur=` / `#fps=` / `#w=` / `#h=` / `#ar=` / `#fc=` / `#res=` / `#ext=` / `#name=` (plus long forms like `#framerate=` / `#width=` / `#height=` / `#frames=` / `#resolution=` / `#format=` / `#file=` / `#filename=`) also keep root-level suggestions visible for quick recovery (including range presets like `#src=2k..4k`, `#dur=1..2`, `#dur=..2`, `#fps=24..60`, `#ar=1.3..1.8`, `#fc=25..200`).
