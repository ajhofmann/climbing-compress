...
- Recent filter also supports metadata tags: `#cached`, `#uncached`, `#out`, `#noout`, `#short`, `#long`, `#portrait`, `#landscape`, `#square` (and exclusions like `-#out` or `!#out`), with simple-tag aliases like `#cache/#warm` -> `#cached`, `#nocache/#cold` -> `#uncached`, `#vertical/#vert` -> `#portrait`, `#horizontal/#horiz` -> `#landscape`, and `#sq` -> `#square`.
...
- Storage comparators are supported with byte units (for example `#src>3k`, `#src>3kb`, `#src>3kib`, `#src!=3k`, `#src>10m`, `#mb>0b`, `#mb>1mb`, `#mb>1mib`), including aliases `#source...` / `#sourcebytes...` (source bytes) and `#render...` / `#outputbytes...` (rendered-output bytes).
...
- Video metadata comparators are supported for FPS/width/height/aspect/frame-count/pixel-area/resolution too (for example `#fps<=24`, `#fps=24..60`, `#w=..1080`, `#h>=1080`, `#ar=1.3..1.8`, `#ar>=16:9`, `#fc<=30`, `#px>=2mp`, `#px=1mp..3mp`, `#res=1920x1080`, `#res>=1920x1080`), including long aliases `#framerate...`, `#width...`, `#height...`, `#aspect...`, `#ratio...`, `#frames...`, `#pixels...`, `#mp...`, and `#resolution...`.
- Resolution comparators accept `x`, `×`, `*`, or `:` separators (for example `#res=1920x1080`, `#res>=1920*1080`, `#res=16:9`) plus `< <= > >= = !=` operators, and support resolution ranges like `#res=1280x720..1920x1080` and open-ended `#res=..1920x1080`.
...
- Malformed `#fps...` / `#w...` / `#h...` / `#ar...` / `#fc...` / `#px...` / `#res...` comparator tags now surface clickable video-meta examples (`#fps>=24`, `#fps=24..60`, `#w>=1080`, `#h>=1080`, `#ar>=1.3`, `#fc>=25`, `#px>=2mp`, `#res=1920x1080`, `#res>=1920x1080`), preserving long-form aliases when typed (`#framerate...`, `#width...`, `#height...`, `#aspect...`, `#ratio...`, `#frames...`, `#pixels...`, `#mp...`, `#resolution...`).
...
- Comparator fragments like `#src=` / `#mb=` / `#out=` / `#dur=` / `#fps=` / `#w=` / `#h=` / `#ar=` / `#fc=` / `#px=` / `#res=` / `#ext=` / `#name=` / `#id=` (plus long forms like `#source=` / `#sourcebytes=` / `#render=` / `#outputbytes=` / `#outputs=` / `#time=` / `#duration=` / `#framerate=` / `#width=` / `#height=` / `#aspect=` / `#ratio=` / `#frames=` / `#pixels=` / `#mp=` / `#resolution=` / `#format=` / `#file=` / `#filename=` / `#video=` / `#videoid=` / `#vid=`) also keep root-level suggestions visible for quick recovery (including range presets like `#src=2k..4k`, `#dur=1..2`, `#dur=..2`, `#fps=24..60`, `#ar=1.3..1.8`, `#fc=25..200`, `#px=1mp..3mp`, `#res=1280x720..1920x1080`).
