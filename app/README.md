...
- comparator fragments like `#src=` / `#out=` / `#dur=` / `#fps=` / `#w=` / `#h=` / `#ar=` / `#fc=` / `#res=` / `#ext=` / `#name=` (plus long forms `#time=` / `#duration=` / `#framerate=` / `#width=` / `#height=` / `#aspect=` / `#ratio=` / `#frames=` / `#resolution=` / `#format=` / `#file=` / `#filename=`) keep root suggestions visible for fast correction (including range presets like `#src=2k..4k`, `#dur=1..2`, `#dur=..2`, `#fps=24..60`, `#ar=1.3..1.8`, `#fc=25..200`)
