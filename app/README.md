...
- video metadata comparators are supported for fps/width/height/aspect/frame-count too (e.g. `#fps<=24`, `#fps=24..60`, `#w=..1080`, `#h>=1080`, `#ar=1.3..1.8`, `#ar>=16:9`, `#fc<=30`), including long aliases `#width...`, `#height...`, and `#frames...`
...
- malformed `#fps...` / `#w...` / `#h...` / `#ar...` / `#fc...` comparator tags show clickable video-meta examples (`#fps>=24`, `#fps=24..60`, `#w>=1080`, `#h>=1080`, `#ar>=1.3`, `#fc>=25`) and preserve long-form aliases (`#width...`, `#height...`, `#frames...`) when typed
...
- comparator fragments like `#src=` / `#out=` / `#dur=` / `#fps=` / `#w=` / `#h=` / `#ar=` / `#fc=` (plus long forms `#width=` / `#height=` / `#frames=`) keep root suggestions visible for fast correction (including range presets like `#src=2k..4k`, `#dur=1..2`, `#dur=..2`, `#fps=24..60`, `#ar=1.3..1.8`, `#fc=25..200`)
