...
export function VideoPlayer() {
  const { outputId, previewId, comparisonId, isRendering, setPlaybackTime } = useStore();
  ...
  const hasComparison = !!comparisonId && !!outputId;

  // Track playback position → store (for chart tracker)
  const onTimeUpdate = useCallback(() => {
    const vid = smartRef.current ?? singleRef.current ?? previewRef.current;
    if (vid) setPlaybackTime(vid.currentTime);
  }, [setPlaybackTime]);

  // Reset playback time when output changes
  useEffect(() => { setPlaybackTime(0); }, [outputId, previewId, setPlaybackTime]);
  ...
  if (!previewId && !outputId) return null;

  return (
    <div className="flex flex-col gap-4">
      {previewId && (
        <div className="flex flex-col gap-1">
          <span className="text-[10px] font-pixel text-center uppercase tracking-[0.2em]" style={{ color: "var(--text-muted)" }}>
            preview render
          </span>
