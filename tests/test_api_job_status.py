...
def test_job_status_non_dict_payloads(tmp_path, monkeypatch):
    db_path = tmp_path / "job-status-non-dict.db"
    input_dir = tmp_path / "input"
    output_dir = tmp_path / "output"
    input_dir.mkdir(parents=True, exist_ok=True)
    output_dir.mkdir(parents=True, exist_ok=True)
    ...
    conn = sqlite3.connect(db_path)
    cur = conn.cursor()
    cur.execute("UPDATE jobs SET request_json = ?, result_json = ? WHERE id = ?", ("[1, 2]", "[3, 4]", "job-status"))
    conn.commit()
    conn.close()
