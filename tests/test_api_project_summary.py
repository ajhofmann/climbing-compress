...
def test_project_summary_api_includes_output_duration(tmp_path, monkeypatch):
    ...
    cur.execute(
        ...
        (json.dumps({"output_duration": "6.1"}), "output-summary-2"),
    )
    conn.commit()
    conn.close()
...
def test_project_summary_invalid_stats(tmp_path, monkeypatch):
    ...
    cur.execute("UPDATE outputs SET created_at = ? WHERE id = ?", (10.0, "output-valid"))
    cur.execute("UPDATE outputs SET created_at = ? WHERE id = ?", (20.0, "output-invalid"))
    conn.commit()
    conn.close()
    ...
    assert response.status_code == 200
    summary = response.json()
    assert summary["latest_output"]["id"] == "output-invalid"
...
def test_project_summary_non_dict_stats(tmp_path, monkeypatch):
    ...
    cur.execute("UPDATE outputs SET created_at = ? WHERE id = ?", (10.0, "output-valid"))
    cur.execute("UPDATE outputs SET created_at = ? WHERE id = ?", (20.0, "output-list"))
    conn.commit()
    conn.close()
