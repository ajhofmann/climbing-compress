...
def test_solve_requires_cache_when_cache_exists(tmp_path, monkeypatch):
    db_path = tmp_path / "solve-cache-present.db"
    input_dir = tmp_path / "input"
    output_dir = tmp_path / "output"
    input_dir.mkdir(parents=True, exist_ok=True)
    output_dir.mkdir(parents=True, exist_ok=True)
    ...
    monkeypatch.setenv("INPUT_DIR", str(input_dir))
    monkeypatch.setenv("OUTPUT_DIR", str(output_dir))
    monkeypatch.setenv("CACHE_VERSION", f"test-solve-present-{tmp_path.name}")
    ...
    monkeypatch.setattr(server_module, "load_analysis", lambda _path: None)
