...
def test_worker_marks_non_dict_payload_failed(tmp_path, monkeypatch):
    db_path = tmp_path / "worker-non-dict.db"
    input_dir = tmp_path / "input-non-dict"
    output_dir = tmp_path / "output-non-dict"
    input_dir.mkdir(parents=True, exist_ok=True)
    output_dir.mkdir(parents=True, exist_ok=True)
    ...
    db_module.init_db()
    db_module.register_video(
        video_id="video-worker-non-dict",
        filename="worker-non-dict.mp4",
        path="/tmp/worker-non-dict.mp4",
        file_hash="hash-worker-non-dict",
    )
    db_module.insert_job(
        job_id="job-non-dict",
        video_id="video-worker-non-dict",
    ...
    job = db_module.get_job("job-non-dict")
    assert job is not None
    worker_module._handle_job(job)

    updated = db_module.get_job("job-non-dict")
    assert updated is not None
    assert updated["status"] == "failed"
    assert updated["message"] == "Invalid request payload: expected object"
...
def test_worker_handles_missing_request_fields(tmp_path, monkeypatch):
    db_path = tmp_path / "worker-missing-request.db"
    input_dir = tmp_path / "input-missing-request"
    output_dir = tmp_path / "output-missing-request"
    input_dir.mkdir(parents=True, exist_ok=True)
    output_dir.mkdir(parents=True, exist_ok=True)

    video_path = tmp_path / "missing-request.mp4"
    video_path.write_bytes(b"")
    ...
    db_module.init_db()
    db_module.register_video(
        video_id="video-missing-request",
        filename=video_path.name,
        path=str(video_path),
        file_hash="hash-missing-request",
    )
    db_module.insert_job(
        job_id="job-missing-request",
        video_id="video-missing-request",
        job_type="analyze",
        status="queued",
    )
    ...
    job = db_module.get_job("job-missing-request")
    assert job is not None
    worker_module._handle_job(job)

    updated = db_module.get_job("job-missing-request")
    assert updated is not None
    assert updated["status"] == "failed"
...
def test_worker_handles_missing_preview_request_fields(tmp_path, monkeypatch):
    db_path = tmp_path / "worker-missing-preview.db"
    input_dir = tmp_path / "input-missing-preview"
    output_dir = tmp_path / "output-missing-preview"
    input_dir.mkdir(parents=True, exist_ok=True)
    output_dir.mkdir(parents=True, exist_ok=True)

    video_path = tmp_path / "missing-preview.mp4"
    video_path.write_bytes(b"")
    ...
    db_module.init_db()
    db_module.register_video(
        video_id="video-missing-preview",
        filename=video_path.name,
        path=str(video_path),
        file_hash="hash-missing-preview",
    )
    db_module.insert_job(
        job_id="job-missing-preview",
        video_id="video-missing-preview",
        job_type="preview",
        status="queued",
    )
    ...
    job = db_module.get_job("job-missing-preview")
    assert job is not None
    worker_module._handle_job(job)

    updated = db_module.get_job("job-missing-preview")
    assert updated is not None
    assert updated["status"] == "failed"
    assert "validation error" in updated["message"].lower()
