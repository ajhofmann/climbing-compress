...
def test_cancel_job_endpoint_noop_for_failed(tmp_path, monkeypatch):
    db_path = tmp_path / "cancel-failed.db"
    input_dir = tmp_path / "input"
    output_dir = tmp_path / "output"
    input_dir.mkdir(parents=True, exist_ok=True)
    output_dir.mkdir(parents=True, exist_ok=True)
    ...
    client = TestClient(server_module.app)
    response = client.post("/api/jobs/job-failed/cancel")
    assert response.status_code == 200
    assert response.json()["status"] == "failed"

    status_response = client.get("/api/jobs/job-failed")
    assert status_response.status_code == 200
...
def test_cancel_job_endpoint_marks_cancelled(tmp_path, monkeypatch):
    db_path = tmp_path / "cancel-update.db"
    input_dir = tmp_path / "input"
    output_dir = tmp_path / "output"
    input_dir.mkdir(parents=True, exist_ok=True)
    output_dir.mkdir(parents=True, exist_ok=True)
    ...
    job = db_module.get_job("job-cancel")
