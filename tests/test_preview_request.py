...
def test_build_preview_request_start_after_duration(tmp_path, monkeypatch):
    db_path = tmp_path / "preview-after.db"
    input_dir = tmp_path / "input"
    output_dir = tmp_path / "output"
    input_dir.mkdir(parents=True, exist_ok=True)
    output_dir.mkdir(parents=True, exist_ok=True)
    ...
    preview_req = server_module._build_preview_request(req, Path("preview.mp4"))
    assert preview_req.trim_start == 2.0
    assert preview_req.trim_end == 4.0


def test_build_preview_request_duration_shorter_than_preview(tmp_path, monkeypatch):
    db_path = tmp_path / "preview-short.db"
    input_dir = tmp_path / "input"
    output_dir = tmp_path / "output"
    input_dir.mkdir(parents=True, exist_ok=True)
    output_dir.mkdir(parents=True, exist_ok=True)
    ...
    monkeypatch.setattr(server_module, "get_video_info", lambda _path: {"duration": 1.5})
    ...
    preview_req = server_module._build_preview_request(req, Path("preview.mp4"))
    assert preview_req.trim_start == 0.2
    assert preview_req.trim_end == 1.5
